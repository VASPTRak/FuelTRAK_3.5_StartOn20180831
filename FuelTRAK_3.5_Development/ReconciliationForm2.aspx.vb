...
Partial Class ReconciliationForm2
    ...
    Public Sub BindGrid()
        ...
        Try
            ...
            Dim dsDisp2 As DataSet
            dsDisp2 = dal.ExecuteStoredProcedureGetDataSet("USP_TT_TANKRECON_GETDATA_DFW_2", parcollection)
            '''dsDisp2 = dal.ExecuteStoredProcedureGetDataSet("USP_TT_TANKRECON_GETDATA_DFW_2_TESTING", parcollection)

            Dim dsDispencers As DataSet
            dsDispencers = dal.ExecuteStoredProcedureGetDataSet("USP_TT_TANKRECON_GETDATA_DFW_DISPENSERS", parcollection)
            'dsDispencers = dal.ExecuteStoredProcedureGetDataSet("USP_TT_TANKRECON_GETDATA_DFW_DISPENSERS_VIEW", parcollection)

            If dsDispencers.Tables(0).Rows.Count > 0 Then
                gvReconciliation.DataSource = dsDispencers
    ...
    Public Sub UpdateTotalizerRecords()
        Try
            ...
            Dim blnDSP2InsertFlag As Boolean = DL.ExecuteStoredProcedureGetBoolean("usp_tt_TankRecon_DFW_2", parcollection)

            'Added By Varun Moota a Temporary Solution to Update NULL or 0.0 in TANK Levels.10/19/2010

            ' ''Dim qry As String = " select * from TANKTOT where isnull (TANK_LEVELS,0) = 0 "

            ' ''Dim dsFRTD As New DataSet
            ' ''dsFRTD = DL.GetDataSet(qry)
            ' ''If dsFRTD.Tables(0).Rows.Count > 0 Then
            ' ''    Dim i As Integer
            ' ''    For i = 0 To dsFRTD.Tables(0).Rows.Count - 1

            ' ''        blnInsertFlag = DL.ExecuteStoredProcedureGetBoolean("usp_tt_TankRecon_DFW")
    ...
    Public Sub CalTankRecon()
        Try
            Dim dl As New GeneralizedDAL
            'Column A Previous Month Totalizer #1 Count
            ...
            Dim strPrevMntTot1 As String = ""
            Dim strPrevMntTot2 As String = ""
            dsResult = dl.GetDataSet(qry)

            If dsResult.Tables(0).Rows.Count > 0 Then
                strPrevMntTot1 = dsResult.Tables(0).Rows(0)("PUMP_TOTALIZERS").ToString()

            Else
                strPrevMntTot1 = 0.0
            End If
            txtPrevMntAmtCnt.Text = strPrevMntTot1.ToString.Trim()
            Session("strPrevMntTot") = strPrevMntTot1.ToString.Trim()


            'Column B Previous Month Totalizer #2 Count
            qry = "select Top (1)  [PUMP_TOTALIZERS]  from tankTot2 " & _
                                "where datepart(mm,[datetime]) = datepart(mm,dateAdd(mm,-1,'" + Session("StartDTRecon").ToString() + "'))" & _
                                 "AND datepart(yy,[datetime]) =  datepart(yy,dateAdd(mm,-1,'" + Session("StartDTRecon").ToString() + "'))" & _
                                " AND TANK_NBR = '" + Session("TNKNBR").ToString() + "' order by [DateTime] desc"
            ...
            If dsResult.Tables(0).Rows.Count > 0 Then
                strPrevMntTot2 = dsResult.Tables(0).Rows(0)("PUMP_TOTALIZERS").ToString()

            Else
                strPrevMntTot2 = 0.0
            End If
            txtPrevMntAmtCnt2.Text = strPrevMntTot2.ToString.Trim()
            Session("strPrevMntTot2") = strPrevMntTot2.ToString.Trim()


            'Column D Previous Month Tank Volume Count
            ...
            'Current Month Totalizer # 1 Count
            qry = "select Top (1)  [PUMP_TOTALIZERS]  from TankTot " & _
            ...
            Dim strCurrentMntTot1 As String = ""
            dsResult = dl.GetDataSet(qry)

            If dsResult.Tables(0).Rows.Count > 0 Then
                strCurrentMntTot1 = dsResult.Tables(0).Rows(0)("PUMP_TOTALIZERS").ToString()

            Else
                strCurrentMntTot1 = 0.0
            End If

            Session("strCurrentMntTot") = strCurrentMntTot1.ToString.Trim()

            txtCntColA.Text = (CDec(strCurrentMntTot1) - CDec(strPrevMntTot1)).ToString()
            Session("CurrTotalizer") = txtCntColA.Text


            'Current Month Totalizer # 2 Count
            qry = "select Top (1)  [PUMP_TOTALIZERS]  from TankTot2 " & _
                    "where datepart(mm,[datetime]) = datepart(mm,'" + Session("StartDTRecon").ToString() + "')" & _
                "And datepart(yy,[datetime]) =  datepart(yy,'" + Session("StartDTRecon").ToString() + "') AND [PUMP_TOTALIZERS] > 0" & _
                "AND TANK_NBR = '" + Session("TNKNBR").ToString() + "' order by [DateTime] desc"

            Dim strCurrentMntTot2 As String = ""
            dsResult = dl.GetDataSet(qry)

            If dsResult.Tables(0).Rows.Count > 0 Then
                strCurrentMntTot2 = dsResult.Tables(0).Rows(0)("PUMP_TOTALIZERS").ToString()

            Else
                strCurrentMntTot2 = 0.0
            End If

            Session("strCurrentMntTot2") = strCurrentMntTot2.ToString.Trim()

            txtCntColB.Text = (CDec(strCurrentMntTot2) - CDec(strPrevMntTot2)).ToString()
            Session("CurrTotalizer2") = txtCntColB.Text

            txtCntColAB.Text = (CDec(Session("CurrTotalizer").ToString) + CDec(Session("CurrTotalizer2").ToString)).ToString()
            Session("CntCurrentTotalizerAB") = txtCntColAB.Text
            ...
            Else
                strCurrentMntDel = "0"
            End If
            txtCntColC.Text = strCurrentMntDel.ToString.Trim()
            ...
            End If
            txtCntColD.Text = strCurrentMntLevels.ToString.Trim()
    ...
    Public Sub TotalsNow()
        Try
            ...
            txtLine1C2.Text = Session("CntCurrentTotalizerAB").ToString()
            ...
            'Line 2 Totatls
            txtLine2C1.Text = Session("CntCurrentTotalizerAB").ToString()
